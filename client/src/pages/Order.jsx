        ...
        <div className="min-h-screen pt-24 px-4 md:px-8 pb-24 md:pb-10 transition-colors duration-300">
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="flex md:grid md:grid-cols-12 gap-0 md:gap-8 overflow-x-auto md:overflow-visible snap-x snap-mandatory md:snap-none scrollbar-hide pb-8 -mx-4 md:mx-0">
                    ...
                    {/* --- PANEL 2: ORDER LIST --- */}
                    <div ref={orderListRef} className="min-w-full md:min-w-0 md:col-span-5 snap-center px-4 md:px-0 pt-16 md:pt-0 flex flex-col h-full lg:col-span-5">
                        {/* Order Summary Card */}
                        <div className="card-3d p-6 md:p-8 bg-white dark:bg-[#1a1c23] flex-1 flex flex-col h-full md:min-h-[500px] lg:bg-white/80 lg:dark:bg-white/5 lg:backdrop-blur-xl lg:border lg:border-gray-200 lg:dark:border-white/10 lg:rounded-3xl lg:p-6 lg:shadow-xl lg:sticky lg:top-24">
                            <h2 className="text-2xl font-black text-gray-800 dark:text-white mb-6 flex items-center gap-3 lg:text-xl lg:mb-4">
                                <ShoppingCart className="text-purple-600 lg:text-blue-500" />
                            ...
                            <div className="flex-1 overflow-y-auto space-y-4 max-h-[50vh] md:max-h-[400px] custom-scrollbar scroll-smooth pr-2 lg:space-y-3 lg:mb-6 lg:max-h-[40vh]">
                                {cart.length === 0 ? (
                                    ...
                                    <AnimatePresence>
                                        {cart.map((item) => (
                                            <motion.div
                                                ...
                                                key={item.id}
                                                className="bg-gray-50 dark:bg-gray-800 p-4 rounded-2xl relative group border border-transparent hover:border-purple-500/30 transition-all lg:p-3 lg:bg-gray-50 lg:dark:bg-white/5 lg:rounded-xl lg:border lg:border-gray-100 lg:dark:border-white/5"
                                            >
                                                <button
                                                    onClick={() => removeFromCart(item.id)}
                                                    className="absolute -top-2 -right-2 bg-red-500 text-white p-1.5 rounded-full shadow-lg opacity-0 group-hover:opacity-100 transition-opacity transform hover:scale-110 z-10 lg:top-2 lg:right-2 lg:p-1 lg:text-gray-400 lg:hover:text-red-500 lg:opacity-0 lg:group-hover:opacity-100"
                                                >
                                                    <Trash2 size={12} />
                                                </button>

                                                <div className="flex items-start justify-between mb-2 lg:mb-1">
                                                    <div className="flex items-center gap-3 overflow-hidden">
                                                        <div className="bg-white dark:bg-gray-700 p-2 rounded-lg">
                                                            <FileText size={20} className="text-blue-500" />
                                                        </div>
                                                        <div>
                                                            <h4 className="font-bold text-gray-800 dark:text-gray-100 truncate max-w-[150px] lg:text-sm">{item.file.name}</h4>
                                                    ...
                                                    <span className="font-mono font-bold text-purple-600 dark:text-purple-400 lg:text-blue-600 lg:dark:text-blue-400">₹{item.cost}</span>
                                                </div>
                                            </motion.div>
                                        ))}
                                    </AnimatePresence>
                            ...
                            <div className="mt-6 pt-6 border-t border-gray-100 dark:border-gray-800 lg:border-t lg:border-gray-100 lg:dark:border-white/10 lg:pt-4 lg:space-y-4">
                                <div className="flex justify-between items-end mb-6 lg:text-gray-600 lg:dark:text-gray-400 lg:mb-0">
                                    <span className="text-sm font-bold text-gray-500 uppercase tracking-widest lg:text-sm">{t('subtotal') || 'Subtotal'}</span>
                                    <div className="text-right">
                                        <span className="text-4xl font-black text-gray-800 dark:text-white block leading-none lg:text-2xl">
                                ...
                                <Button
                                    ...
                                    className={`w-full py-4 text-xl shadow-2xl shadow-purple-500/30 bg-gradient-to-r from-purple-600 to-pink-600 ${cart.length === 0 ? 'opacity-50 cursor-not-allowed' : 'hover:scale-[1.02] active:scale-95'} lg:w-full lg:py-4 lg:rounded-xl lg:font-bold lg:shadow-lg lg:shadow-green-500/20 lg:bg-gradient-to-r lg:from-green-500 lg:to-emerald-600 lg:hover:from-green-600 lg:hover:to-emerald-700 lg:border-none`}
                                >
                    ...
                    <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-[#111] border-t border-gray-200 dark:border-white/10 p-4 pb-6 z-50 shadow-[0_-5px_15px_-5px_rgba(0,0,0,0.1)] flex items-center gap-4">
                        <div className="flex-1">
                            <div className="text-xs text-gray-500 dark:text-gray-400 uppercase font-bold tracking-wider">Total</div>
                            <div className="text-2xl font-black text-gray-900 dark:text-white">₹{calculateGrandTotal()}</div>
                        </div>
                        {cart.length > 0 ? (
                            <Button
                                onClick={handleSubmitOrder}
                                className="flex-1 py-3 rounded-xl font-bold shadow-lg shadow-green-500/20 bg-gradient-to-r from-green-500 to-emerald-600 border-none text-sm"
                                disabled={loading}
                            >
                            ...
                            <Button
                                onClick={addToCart}
                                className="flex-1 py-3 rounded-xl font-bold shadow-lg shadow-blue-500/20 bg-blue-600 text-sm"
                                disabled={!currentFile}
                            >
                                + Add Item ₹{calculateCurrentItemCost()}
                            </Button>
                        )}
                    </div>
