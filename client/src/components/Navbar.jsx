...
const Navbar = () => {
    ...
    return (
        <>
            {/* --- MOBILE NAVBAR (Strict 56px, Sticky, Glass) --- */}
            <nav className="md:hidden sticky top-0 z-50 h-[56px] w-full bg-[#FFFFFFCC] dark:bg-[#0F172ACC] backdrop-blur-xl border-b border-black/5 dark:border-white/5 flex items-center justify-between px-4 transition-colors duration-300 shadow-[0_8px_20px_rgba(0,0,0,0.06)] dark:shadow-[0_8px_20px_rgba(0,0,0,0.4)]">
                {/* Left: Logo (32x32) */}
                <Link to="/" className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-lg bg-gradient-to-tr from-blue-600 to-purple-600 flex items-center justify-center text-white shadow-lg">
                        <span className="font-black text-[10px]">SX</span>
                    </div>
                </Link>

                {/* Center: App Name */}
                <span className="text-lg font-black bg-gradient-to-r from-gray-900 to-gray-700 dark:from-white dark:to-gray-200 bg-clip-text text-transparent absolute left-1/2 -translate-x-1/2">
                    Smart Xerox
                </span>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button
                        onClick={toggleTheme}
                        className="w-8 h-8 rounded-full bg-black/5 dark:bg-white/10 flex items-center justify-center text-gray-600 dark:text-yellow-400"
                    >
                        {theme === 'light' ? <Moon size={16} /> : <Sun size={16} />}
                    </button>

                    {/* Mobile Menu Trigger (Placeholder for now) */}
                    <button className="w-8 h-8 flex items-center justify-center text-gray-800 dark:text-white">
